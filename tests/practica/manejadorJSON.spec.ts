...
import { ManejadorJSON } from "../../src/practica/manejadorJSON.js";
import { Funko } from "../../src/practica/funko.js";
import { TipoFunko, GeneroFunko } from "../../src/practica/enumerados.js";
...
describe("Clase ManejadorJSON ", () => {
  ...
  it("listarFunkoDB : método que muestra por pantalla los funkos de un usuario", () => {
    expect(ManejadorJSON.listarFunkoDB('facundo', true)).to.be.equal(true);
    expect(ManejadorJSON.listarFunkoDB('pepe', true)).to.be.equal(false);
  });
  it("mostrarFunkoDB : método que muestra por pantalla un funko de un usuario", () => {
    expect(ManejadorJSON.mostrarFunkoDB(4, 'facundo', true)).to.be.equal(true);
    expect(ManejadorJSON.mostrarFunkoDB(5, 'facundo', true)).to.be.equal(false);
  });
});
